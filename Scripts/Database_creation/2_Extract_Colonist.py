...
def traiter_article(lignes_article, donnees_articles):
    # Initial defaults for the variables
    type_nouvelle, titre, auteur, nombre_mots, date, langue, numero_page, texte = ('Inconnu',) * 8
    ...
    if index_ligne_vide is not None and index_ligne_vide > 1:
        ...
        if index_ligne_vide - 1 < len(lignes_article):
        ...
        if len(lignes_article) > 0:
            type_nouvelle = lignes_article[0]
        if len(lignes_article) > 1:
            titre = lignes_article[1]
        if len(lignes_article) > 3:
            auteur = lignes_article[3]
    ...
    indice_fin = next((i for i, ligne in enumerate(lignes_article) if "copyright" in ligne.lower()), None)
...
chemin_fichier = '/Volumes/CLIMATE.FRAME/Raw_data/Times_Colonist/Times_Colonist_clean_data/combined_text.txt'
...
chemin_csv = '//Volumes/CLIMATE.FRAME/Raw_data/Times_Colonist/Times_Colonist_clean_data/articles_extraits.csv'
