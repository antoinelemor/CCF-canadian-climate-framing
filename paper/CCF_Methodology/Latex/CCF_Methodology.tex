...
The CCF Database is deposited on Zenodo under a CC-BY~4.0 licence in two complementary editions sharing identical schemas. The canonical PostgreSQL edition (\url{https://doi.org/10.5281/zenodo.20346364}~\autocite{lemor_ccf_database_2026}) ships as a single tarball (\texttt{CCF\_Database.tar}, $\approx$~37~GB) wrapping a \texttt{pg\_dump} \emph{directory} archive of the six relational tables together with all B-tree, partial, and HNSW indexes; restoration requires PostgreSQL~16 or~17 with the \texttt{pgvector} extension ($\geq$~0.8.2 for \texttt{halfvec} support) and the deposit's README provides the exact three-line restore command (\texttt{tar -xf}, \texttt{createdb}, \texttt{pg\_restore -j 8}). A sister column-oriented Apache Parquet mirror (\url{https://doi.org/10.5281/zenodo.20346373}~\autocite{lemor_ccf_database_parquet_2026}, one ZSTD-compressed file per table, $\approx$~15.8~GB) is provided for users without a PostgreSQL backend; the \texttt{halfvec(1024)} embedding column is materialised as a 1024-element \texttt{LIST<FLOAT>} and the JSONB entity arrays are serialised as UTF-8 JSON strings, so each Parquet file can be read directly with \texttt{pandas}, \texttt{polars}, R/\texttt{arrow}, DuckDB, or Spark. The companion code, documentation, and reproducible pipeline---including the annotation pipeline used to build the database (\texttt{Scripts/Annotation/}), the canonical CSVs underlying the supplementary tables (\texttt{Database/Training\_data/}), and the full training data in JSONL format---are bundled with both Zenodo editions as \texttt{ccf\_code\_and\_paper.tar.gz} and additionally archived on the Open Science Framework at \url{https://doi.org/10.17605/OSF.IO/Q5W47}~\autocite{lemor_ccf_osf_2026}, with a regularly updated development mirror on GitHub at \url{https://github.com/antoinelemor/CCF-canadian-climate-framing}.
