...
\begin{abstract}
\noindent
News media shape how societies perceive and respond to climate change, yet sentence-level analysis of climate discourse has been hindered by fragmented corpora and copyright restrictions. The Canadian Climate Framing (CCF) database releases a fully annotated corpus of 266,271 climate-related articles published in 20 Canadian newspapers between 1978 and 2024, processed into 9,198,958 two-sentence units (82.9\% English, 17.1\% French). Each unit carries 65 binary annotations spanning thematic frames, actors, events, solutions, tone, geographic focus, and urgency, produced by 128 BERT and CamemBERT classifiers trained on 4{,}000+ expert-coded sentences. Models reach a macro $F_1$ of 0.866 on a stratified 1{,}000-sentence gold standard double-coded by an independent annotator (Gwet's AC1 $=$ 0.894). The deposit ships six relational tables on Harvard Dataverse (CC-BY 4.0) as a \texttt{pg\_dump} archive and as an Apache Parquet bundle: annotations, named entities, article-level aggregates, reliability tiers, and 9{,}462{,}845 BAAI/bge-m3 sentence embeddings. The annotation framework, training data, and reproducible pipeline are released so that the CCF can be extended elsewhere.
\end{abstract}
...
On February 18, 1978, \textit{The Globe and Mail} ran a story headlined ``Some think it's warming up'', reporting on a survey of leading climate experts. Almost half a century ago, the article already described climate change in terms that have since become very familiar:
\begin{quote}
...
\end{quote}
This excerpt illustrates the role the media play in informing society and citizens---most of whom do not have direct access to academic research---and at the same time reflecting the social and political realities of the country they cover. Research shows that media shape not only what people think about but also how they think about it, with effects on both public opinion and political agendas \autocite{scheufele_framing_2007, walgrave_contingency_2006}. The media are accordingly not a neutral arena for public debate around climate change. Instead, they actively participate in defining and constructing the very problems under discussion \autocite{cohen_folk_1972, trevino_media_2018}. Moreover, while digital and social media have grown in prominence, newspapers remain a foundational source of information that continues to shape public discourse and to influence other media formats \parencite{bastos_shares_2015, casero-ripolles_impact_2020}. Studying print media therefore provides a vantage point on a traditional and still highly influential channel of climate communication, but most of all it allows for long-term historical comparison, including periods that predate the rise of social media.

The excerpt above was drawn from the oldest article of the \emph{Canadian Climate Framing (CCF)} database, which we introduce in this Data Descriptor: a machine-learning-preprocessed corpus of 266,271 climate-related newspaper articles published between 1978 and 2024 across 20 major Canadian outlets, accompanied by a rigorous, theoretically grounded annotation framework that we release alongside the data. The corpus contains 9,198,958 two-sentence analytical units, all annotated within a 65-category hierarchical coding scheme. Because Canadian newspaper articles are protected by copyright, the deposit does not redistribute raw sentence text. What is shared instead is (i) the 65-category hierarchical annotation framework---a theoretically grounded coding scheme that integrates thematic frames, actors, events, solutions, emotional tone, geographic focus, and urgency into a single sentence-level taxonomy---and (ii) the full output of an intensive annotation pipeline built around that framework: 65 binary sentence-level labels per analytical unit, the named-entity extractions (persons, organizations, locations), the article-level aggregates and full metadata (newspaper, date, title, author, page number), the per-category reliability tiers, and the BAAI/bge-m3 sentence embeddings (9.46~million 1024-dimensional vectors). The breadth of the annotation scheme and the embedding layer together make the deposit fully analysable at the sentence level without requiring access to the raw text. By focusing on Canadian print media, this database constitutes a historically influential source of climate information.
...
Articles published by external news agencies (for example, \textit{Agence France-Presse}, \textit{CanWest News Service}, \textit{Reuters}, or \textit{Bloomberg}) or by other newspapers not included in the analysis but which often belong to the same media conglomerate as the selected outlets (for example, \textit{the Financial Post}) were also retained. These republications were treated as aligned with the editorial direction of the host news outlet, and a republication did not contradict the objectives of the study since it indicated editorial interest in a topic concerning climate change. This process reduced the corpus to 266,271 unique articles. Duplicates of the same article appearing in different media outlets were deliberately kept, since their co-occurrence is itself an object of analysis when measuring media bubbles. Language was then verified with the pre-trained fastText language-identification model \autocite{joulin_bag_2017, joulin_fasttext_2016}, in order to ensure consistent categorisation of bilingual content, and publication dates were standardised at the same step. This data preparation produced a clean, standardised corpus ready for machine learning and annotation, stored in two PostgreSQL tables: one with full metadata (titles, main text, authors, dates, and page numbers) and another with the processed sentence segments and their corresponding identifiers.

The resulting database is illustrated in Figure~\ref{fig:media_dist}, which displays the distribution of article counts by media (20 outlets), while Figure~\ref{fig:year_counts} shows the total article counts per year across the full time span. Data extraction concluded in February 2025, and 2025 was therefore excluded from the yearly counts to avoid reporting on an incomplete year; the authors plan to update the database periodically and develop it into an observatory of Canadian climate discourse. Finally, Figure~\ref{fig:province_dist} presents the distribution of articles by province, based on the primary location of each newspaper. This geographic breakdown shows the representativeness of the database across Canada, with 82.9\% of articles in English and 17.1\% in French. Three national newspapers---\textit{The Globe and Mail}, the \textit{National Post}, and the \textit{Toronto Star}---are omitted from the provincial breakdown because of their pan-Canadian scope; nevertheless, they account for 36.2\% of all articles.
...
Models failing to achieve a positive-class F1 score of at least 0.60 during the training phase underwent an automated reinforced training protocol. This protocol was triggered for 45 of the 128 trained models (35\%), concentrated in low-prevalence sub-frames---cultural, justice, and security---and in the fine-grained political and scientific sub-categories. The reinforced phase implemented weighted random sampling to oversample minority classes, doubled the batch size to 64, reduced the learning rate to $1\times 10^{-5}$, and applied a weighted cross-entropy loss with emphasis on the positive class. Training was extended for an additional 20 epochs, after which the best epochs of the normal and reinforced phases were compared on a combined criterion that gave 70\% weight to the positive-class F1 and 30\% to the macro F1, i.e.\ $0.7 \cdot F_1^{(1)} + 0.3 \cdot \text{macro-}F_1$. The reinforced epoch was retained as the final model for 23 of these 45 cases, while the remaining 22 reverted to their best normal-training epoch; averaged over the 23 reinforced models eventually retained, the positive-class F1 improved by $+0.25$, lifting most of the affected categories above the deployment threshold. Five categories could nevertheless not be trained at all because of insufficient positive annotations in the training data, and were consequently excluded from the annotation framework: \textit{carbon footprint of the health sector}, \textit{positive impacts of climate change on health}, \textit{post-disaster military assistance}, \textit{disruption of military operations due to climate change}, and \textit{carbon footprint of the defense sector}. The complete per-model training provenance, including which models triggered reinforcement and which retained the reinforced epoch, is provided in Supplementary Table~S11.
...
\node[header] (head) {The enriched CCF Database (5 tables, 1 extension)};
\node[orig, below=of head, xshift=-5.4cm] (full)  {\textbf{CCF\_full\_data}\\\textit{266{,}271 rows}\\article-level metadata\\(title, author, date, media, page)};
\node[orig, right=of full] (proc) {\textbf{CCF\_processed\_data}\\\textit{9{,}198{,}958 rows}\\sentence-level\\10 metadata cols + 65 annotations + NER};
\node[aggr, right=of proc] (aggr) {\textbf{CCF\_article\_aggregates}\\\textit{266{,}271 rows}\\65 \texttt{prop\_X} + dominant frame + entropy + sub-counts};
\node[ent,  below=of full]  (ent)  {\textbf{CCF\_article\_entities}\\\textit{266{,}271 rows}\\deduplicated PER / ORG / LOC arrays + per-type counts};
\node[rel,  below=of aggr]  (rel)  {\textbf{CCF\_reliability\_tiers}\\\textit{65 rows}\\category $\rightarrow$ tier A/B/C\\(overall, EN, FR)};
\node[embed,below=of proc]  (embed){\textbf{CCF\_sentence\_embeddings}\\\textit{9{,}462{,}845 rows}\\\texttt{(doc\_id, sentence\_id)} primary key\\BGE-M3 \texttt{halfvec(1024)}\\HNSW index, cosine ops};
...
The CCF Database is deposited on Harvard Dataverse at \url{https://doi.org/10.7910/DVN/0FKOQB}~\autocite{lemor_ccf_database_2026} under a CC-BY~4.0 licence, as a single tarball (\texttt{CCF\_Database.tar}, $\approx$~37~GB) wrapping a \texttt{pg\_dump} \emph{directory} archive of the six relational tables together with all B-tree, partial, and HNSW indexes. Restoring the dataset requires PostgreSQL~16 or~17 with the \texttt{pgvector} extension ($\geq$~0.8.2 for \texttt{halfvec} support); the deposit's README provides the exact three-line restore command (\texttt{tar -xf}, \texttt{createdb}, \texttt{pg\_restore -j 8}). A parallel column-oriented Apache Parquet bundle (one ZSTD-compressed file per table, $\approx$~15.8~GB) is provided alongside the PostgreSQL dump for users without a PostgreSQL backend; both formats share identical schemas, with the \texttt{halfvec(1024)} embedding column materialised as a 1024-element \texttt{LIST<FLOAT>} in Parquet and the JSONB entity arrays serialised as UTF-8 JSON strings, so a single Parquet file can be read directly with \texttt{pandas}, \texttt{polars}, R/\texttt{arrow}, DuckDB, or Spark. The companion code, documentation, and reproducible pipeline---including the annotation pipeline used to build the database (\texttt{Scripts/Annotation/}), the canonical CSVs underlying the supplementary tables (\texttt{Database/Training\_data/}), and the full training data in JSONL format---are archived on the Open Science Framework at \url{https://doi.org/10.17605/OSF.IO/Q5W47}~\autocite{lemor_ccf_osf_2026}, with a regularly updated development mirror on GitHub at \url{https://github.com/antoinelemor/CCF-canadian-climate-framing}.

Because Canadian newspaper articles are protected by copyright, the deposit does not redistribute raw sentence text or full article bodies. What is shared is everything else: article-level metadata (titles, authors, publication dates, newspapers, page numbers), sentence-level identifiers, the 65 binary annotations, the named-entity extractions, the article-level aggregates and entity rollups, the per-category reliability tiers, and the BGE-M3 sentence embeddings. Each article is identified by its bibliographic coordinates (\texttt{media}, \texttt{date}, \texttt{title}, \texttt{author}, \texttt{page\_number}), which is the standard bibliographic reference for Canadian newspapers and is sufficient for any researcher with institutional access to Factiva, Eureka.cc, or ProQuest Canadian Newsstream to retrieve the original text. No DOIs exist for the historical articles covered by the corpus.
...
\midrule
CCF\_full\_data            & 266{,}271     & Article  & Bibliographic metadata for every article in the corpus. \\
CCF\_processed\_data       & 9{,}198{,}958 & Sentence & The 65 sentence-level binary annotations and the per-sentence NER output. \\
CCF\_article\_aggregates   & 266{,}271     & Article  & Per-article proportions, dominant thematic frame, framing entropy and sub-category counts. \\
CCF\_article\_entities     & 266{,}271     & Article  & Deduplicated JSONB arrays of PER / ORG / LOC mentions, with per-type cardinalities and per-type sentence counts. \\
CCF\_reliability\_tiers    & 65            & Category & Per-category reliability tier (A / B / C) with the underlying $F_1$ and $\kappa$ values. \\
CCF\_sentence\_embeddings  & 9{,}462{,}845 & Sentence + title & L2-normalised BGE-M3 1024-dimensional vectors, queryable through \texttt{pgvector}. \\
\bottomrule
\end{tabularx}
\end{table}
...
dominant\_frame      & TEXT             & Thematic frame with the highest \texttt{prop\_X}; NULL when no frame is positive. \\
dominant\_frame\_prop & DOUBLE PRECISION & Proportion of the dominant frame in the article. \\
entropy\_frames      & DOUBLE PRECISION & Shannon entropy (natural base) of the eight thematic-frame proportions; 0 when no thematic frame is positive. \\
...
\textnormal{\textbf{Column}} & \textnormal{\textbf{Type}} & \textbf{Definition} \\
\midrule
number          & SMALLINT (PK)  & Supplementary Table~S3 reference number (1--65). \\
...
\captionof{table}{Article-level descriptive statistics of the deposited CCF Database. Mean share is the average per-article proportion of sentences positive for the category; the dominant-frame column reports the percentage of articles for which the frame has the highest share among the eight thematic frames. Computed from \texttt{CCF\_full\_data}, \texttt{CCF\_processed\_data}, and \texttt{CCF\_article\_aggregates}.}
\medbreak
...
\captionof{figure}{Mean per-article share of each thematic frame, year by year, 1980--2024. Each point is the mean of the corresponding \texttt{prop\_X} column of \texttt{CCF\_article\_aggregates} across the articles published in that year, joined with \texttt{CCF\_full\_data} on \texttt{doc\_id}. The pre-1988 portion of the curve is computed on fewer than fifty articles per year.}
...
\captionof{table}{Ten most frequently mentioned entities of each type in the deposited CCF Database. ``Articles'' is the number of articles in which the entity appears at least once. Computed from the JSONB arrays of \texttt{CCF\_article\_entities} (\texttt{entities\_per}, \texttt{entities\_org}, \texttt{entities\_loc}).}
...
{\bfseries{Article-level descriptive statistics.}} Table~\ref{tab:data_overview_descriptives} reports the corpus size (article count, two-sentence analytical units, and per-article word statistics) computed on \texttt{CCF\_full\_data} and \texttt{CCF\_processed\_data}, and the mean per-article share of sentences positive for each annotation category, computed on the \texttt{prop\_X} columns of \texttt{CCF\_article\_aggregates}. The left block lists the eight thematic frames together with the percentage of articles for which each frame is the \texttt{dominant\_frame}; the right block lists the primary categories (messenger, event, solution, Canadian context, urgency) and the three tones.

{\bfseries{Temporal coverage of the thematic frames.}} Figure~\ref{fig:data_overview_frames_lines} plots, for every calendar year between 1980 and 2024, the mean of each \texttt{prop\_X} thematic-frame column of \texttt{CCF\_article\_aggregates}, grouped by the year extracted from \texttt{CCF\_full\_data.date}. No smoothing or regression is applied; each point is a yearly mean. The earliest years of the corpus carry very few articles---fewer than fifty per year before 1988 and fewer than five hundred per year before 1990---so the pre-1988 portion of the curve is computed on small samples and should be read with that caveat.
...
Two structural constraints were then applied on top of this weighted draw. First, a minimum threshold of 40 positive examples per category was imposed to guarantee sufficient statistical power for reliable performance estimation: without it, extremely rare categories such as \textit{Disruption of military operations} (whose prevalence in the database is on the order of $10^{-5}$) would have had too few examples to support a meaningful assessment of model accuracy. Second, a 35\% maximum allocation per category was imposed to prevent any single prevalent category from monopolising the validation set; without this ceiling, \textit{Actors/Messengers Detection} alone, present in 48.8\% of database sentences, could plausibly have dominated the entire sample and left insufficient space for the rarer categories.
...
All 1,000 sentences in the validation set were manually annotated by an expert annotator specialising in climate change politics and policy. These expert annotations defined the gold standard against which all model performance metrics reported in this section were computed.

The validation results demonstrated robust performance across all annotation dimensions, with an overall F1 macro score of 0.866 (0.869 for English, 0.864 for French). As detailed in Supplementary Table~S7, primary detection categories achieved the highest performance, with \textit{Canadian Context} (F1=0.982) and \textit{Actors/Messengers} (F1=0.961) showing near-perfect classification. Thematic frames maintained strong performance despite their semantic complexity, with \textit{Scientific Frame} (F1=0.890) and \textit{Environmental Frame} (F1=0.871) leading this category. The hierarchical classification strategy proved particularly effective, as evidenced by consistently higher recall scores for detection categories (mean recall=0.891) that triggered conditional sub-category evaluation. Notably, the rarest category in the validation set, \textit{Disruption of military operations}, reached perfect precision on the French subset where two positive examples were available; the broader \textit{Post-disaster military assistance} category, by contrast, had too few positives to support a reliable evaluation in either language and was flagged as exploratory in the reliability tiers (Supplementary Table~S10).

To assess temporal stability, stratified validation was conducted across five temporal periods (1980s--2020s); the analysis revealed minimal performance drift ($\Delta$F1 < 1.1\%) despite evolving media discourse patterns (Figure~\ref{fig:temporal_validation}). This temporal consistency validated the models' generalisation capacity across the full 47-year corpus span.
